'''
Generate synthetic dataset and train SLM using provable algorithm. Show the learning curves along iteration.
...
def train(export_filename):
    ...
    for repeat_count in xrange(repeat_times):
        ...
        the_bias_term = -0.5
        y = X.dot(w_true) + libSLM.A_(X.T, U_true, U_true) + the_bias_term
        y = y.flatten()

        # generate true labels for testing
        y_test = X_test.dot(w_true) + libSLM.A_(X_test.T, U_true, U_true) + the_bias_term
...
if __name__ == '__main__':
    export_filename = './SLM_Provable_Alg_learning_curve.npz'
    ...
    # plot curves
    export_figname = './SLM_Provable_Alg_learning_curve.png'
